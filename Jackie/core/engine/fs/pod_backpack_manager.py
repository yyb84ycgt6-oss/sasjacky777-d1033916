...
class PodBackpackManager:
    ...
    def _register_defaults(self) -> None:
        """
        Registers the built-in pods and backpacks.

        `__init__` called this and the class did not define it — it defines
        `_register_pods` and `_register_backpacks` separately — so constructing
        a PodBackpackManager raised AttributeError every time. Nothing in the
        runtime above it could ever have been started.
        """
        self._register_pods()
        self._register_backpacks()
