...
serve(async (req) => {
  ...
  const un = await gate(req, "jackie-cerebras");
  if (un) return un;

  const key = Deno.env.get(SECRET);
  if (!key) {
    return new Response(JSON.stringify({
      error: `${SECRET} not configured. Get a free key at https://cloud.cerebras.ai/`,
      needs_secret: SECRET,
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return new Response(JSON.stringify({ error: `Cerebras ${resp.status}: ${text}` }), {
