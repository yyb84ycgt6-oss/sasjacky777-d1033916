...
serve(async (req) => {
  ...
  const un = await gate(req, "jackie-google");
  if (un) return un;

  const key = Deno.env.get(SECRET);
  if (!key) {
    return new Response(JSON.stringify({
      error: `${SECRET} not configured. Get a free key at https://aistudio.google.com/apikey`,
      needs_secret: SECRET,
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return new Response(JSON.stringify({ error: `Google ${resp.status}: ${text}` }), {
