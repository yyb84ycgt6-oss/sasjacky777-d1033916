/**
 * Seeded simplex noise, 2D and 3D, after Stefan Gustavson's reference
 * implementation. Each generator owns its own permutation table built from a
 * seed, so terrain, caves, climate and ore placement draw from independent
 * fields instead of the same one sampled at an offset — offsets leave visible
 * correlation (every mountain sitting on a cave system) that players notice.
 */
import { Rng } from "./rng";
