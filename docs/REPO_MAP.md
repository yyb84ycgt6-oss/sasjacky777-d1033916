...
every texture and sound generated by code in `src/craft`. `csc/` builds the
same game on its own: a desktop app (with LAN play) and a single offline
`.html`. See [CSC.md](CSC.md).
